    ...
    def show_progress():
        ...
        while process.poll() is None:
            ...
            if mins > 0:
                ...
                time_str = f"{secs}s"
            print(
                f"\r   {spinner[i % len(spinner)]} {description}... {time_str} elapsed",
                end="",
                flush=True,
            )
            i += 1
...
def preprocess_audio(input_file, output_file):
    ...
    print(
        f"   🔧 Preprocessing (16kHz mono + aggressive silence removal)...", flush=True
    )
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    ...
    if result.returncode == 0 and Path(output_file).exists():
        ...
        if time_saved > 0:
            ...
            print(
                f"   ✅ Removed {time_saved_str} of silence ({percent_saved:.1f}%) • {orig_mins}:{orig_secs:02d} → {proc_mins}:{proc_secs:02d}"
            )
        else:
...
def main():
    ...
    print(
        f"🤖 AI will then read RAW-TEXT.md and create PROCESSING-PLAN.md (using semantic search)"
    )
